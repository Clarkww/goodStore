...
function startGame() {
    ...
    class PowerUp {
      ...
      update(player, projectiles) {
        if (this.active && collide(this, player)) {
          this.active = false;
          this.applyPowerUp(player, projectiles);
        }
      }
    
      applyPowerUp(player, projectiles) {
        if (this.type === "double-shot") {
          player.shoot = () => {
            projectiles.push({
    ...
    function drawProjectiles() {
      for (let i = 0; i < projectiles.length; i++) {
        if (powerupActive && powerUpType === 'doubleShot') {
          // Draw two projectiles at different angles
          ctx.drawImage(loadedImages['./img/bluespell.png'], projectiles[i].x - 10, projectiles[i].y, 10, 30)
          ctx.drawImage(loadedImages['./img/bluespell.png'], projectiles[i].x + 10, projectiles[i].y, 10, 30)
        } else {
    ...
    let shoot = () => {
      ...
      if (powerupActive) {
          // fire two projectiles at different angles
          let projectile1 = {
          ...
          projectiles.push(projectile1, projectile2)
      } else {
